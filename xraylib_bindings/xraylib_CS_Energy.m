function A = xraylib_CS_Energy(Z, E)
...
%
% 
% Given an element Z and an energy E (in keV), returns the mass-energy
% absorption cross section in cm2/g

    xraylib_loadlibrary;
    if numel(E)>1 && numel(Z)==1
        A = cellfun(@xraylib_CS_Energy,num2cell(Z*E.^0),num2cell(E));
    elseif numel(E)==1 && numel(Z)>1
        A = cellfun(@xraylib_CS_Energy,num2cell(Z),num2cell(E*Z.^0));
    elseif numel(E)>1 && numel(Z)>1
        A=zeros(numel(Z),numel(E));
        for i = 1:numel(E)
            A(:,i) = cellfun(@xraylib_CS_Energy,num2cell(Z),num2cell(E(i)*Z.^0));
        end
    else
        A = calllib('libxrl','CS_Energy', Z, E);
    end
%     A = calllib('libxrl','CS_Energy', Z, E);
end