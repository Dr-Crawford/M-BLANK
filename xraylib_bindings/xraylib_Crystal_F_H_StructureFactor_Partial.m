function xrlComplex = xraylib_Crystal_F_H_StructureFactor_Partial(Crystal_Struct, E, i_miller,j_miller, k_miller, debye_factor, rel_angle, f0_flag, f_prime_flag, f_prime2_flag)
...
% re: real part
% im: imaginary part
% Wherever possible for the bindings (Python, IDL, Perl, Ruby, Fortran,
% C#), we have tried using the native complex number datatype in favor of a
% direct analogue of the xrlComplex struct.
% 
% See also xraylib_Crystal_F_H_StructureFactor.
% 
% The Atomic structure factor has three terms:
% 
% FH = f0 + ?f? + ?f?.
% For each of these three terms, there is a corresponding *_flag argument
% which controls the numerical value used in computing FH:
% 
...
% See: Crystal_Struct is the output of xraylib_Crystal_GetCrystal.
    %{
    ...
    %}
    xraylib_loadlibrary;
    xrlComplex = calllib('libxrl','Crystal_F_H_StructureFactor_Partial',Crystal_Struct, E, i_miller,j_miller, k_miller, debye_factor, rel_angle, f0_flag, f_prime_flag, f_prime2_flag);
end