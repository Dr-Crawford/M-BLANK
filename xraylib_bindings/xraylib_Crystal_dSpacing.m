function xrlComplex = xraylib_Crystal_dSpacing(Crystal_Struct, i_miller, j_miller, k_miller)
...
%
% Computes the d?spacing for a given crystal cryst and Miller indices (
% i_miller , j_miller and k_miller ). The routine assumes that if
% cryst?>volume is nonzero then it holds a valid value. If (i,j,k) =
% (0,0,0) then zero is returned.
% See: Crystal_Struct is the output of xraylib_Crystal_GetCrystal.
    ...
    %}
    xraylib_loadlibrary;
    xrlComplex = calllib('libxrl','Crystal_dSpacing',Crystal_Struct, i_miller, j_miller, k_miller);
end