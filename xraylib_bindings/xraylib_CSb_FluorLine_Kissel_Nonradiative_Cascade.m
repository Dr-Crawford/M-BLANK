function A = xraylib_CSb_FluorLine_Kissel_Nonradiative_Cascade(Z, line, E)
...
%
% Given an element Z , a line?type macro line and an energy E, returns the
% XRF cross section expressed in barn/atom calculated using the partial
% photoelectric effect cross sections calculated by Kissel et al. and
% taking into account cascade effects coming from non?radiative
% transitions; but keep in mind that in reality both radiative and
% non-radiative effects will always be occuring simultaneously.
%
% See: xraylib_macro_integer_references
%
% NOTE: All CS_FluorLine* functions offer XRF cross sections for both K?
% and L?lines ?provided the corresponding shells can be excited?, but only
% the CS_FluorLine_Kissel* functions offer also the M?line XRF cross
% sections!
    ...
    %}
    xraylib_loadlibrary;
    A = calllib('libxrl','CSb_FluorLine_Kissel_Nonradiative_Cascade',Z, line, E);
end